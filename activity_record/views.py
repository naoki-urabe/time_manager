...
class ActivityRecordView(View):
    ...
    def get_active_histories(self):
        active_histories = ActiveRecord.objects.filter(today_jst=localtime(timezone.now()))
        return active_histories
    def get_active_id(self,active_exists):
        active_id = -1
        if active_exists:
            print(timezone.now())
            active_id =  ActiveRecord.objects.get(task='active',today_jst=to_jst(timezone.now()),is_active=True).id
        return active_id
    def get_task_exists(self):
        task_exists = TaskRecord.objects.filter(is_active=True).exists()
        return task_exists
    def get_task_histories(self):
        task_histories = TaskRecord.objects.filter(today_jst=localtime(timezone.now()))
...
class RegisterActiveRecord(View):
    def post(self, request, *args, **kwargs):
        ...
        if active_exists == 'True':
            ...
            #print(task_record.end_time-task_record.begin_time)
            ...
            active_record = ActiveRecord(task='active',begin_time=localtime(timezone.now()),today_jst=to_jst(timezone.now()))
            active_record.save()
        context = {
            'active_status': "打刻完了",
            'now': timezone.now()
...
class RegisterTaskRecord(View):
    def post(self, request, *args, **kwargs):
        ...
        if task_id=='-1':
            ...
            task_record = TaskRecord(task=task_name,begin_time=localtime(timezone.now()),today_jst=to_jst(timezone.now()))
            task_record.save()
        else:
            task_record = TaskRecord.objects.get(id=task_id)
            task_record.end_time=localtime(timezone.now())
            print(type(task_record.end_time-task_record.begin_time))
            task_record.period = timedelta_to_sec(task_record.end_time - task_record.begin_time)
            task_record.is_active = False
            task_record.today_jst = to_jst(timezone.now())
            task_record.save()
        context = {
            'active_status': "打刻完了",
            'now': timezone.now()
